...
(function () {
    ...
    <script>
        ...
        // r131/r137: kept in sync with GanapatiAI.ino's own mantraTracks[] -
        // track 14's duration corrected to its real re-recorded length
        // (102000ms, was 175730), and track 10 removed entirely (r137
        // reinstated r132 - it plays as part 2 of the closing Aarti block
        // instead, confirmed as the intended pairing).
        const mantraTracks = [
        ...
        // r139: separate list for mouse-back's own pool (never had a JS
        // mirror at all before this) - real bug found while investigating
        // a direct report that "the mantra number for the same mantra is
        // different": the Now Playing sync below only ever searched
        // mantraTracks (feet's list), so a mouse-back touch's real track
        // number (17/21-29) never matched anything and the panel just
        // kept showing whatever was last correctly matched, stale.
        const mouseChantTracksJS = [
            { file: "0017.mp3", duration: 12000, dfTrack: 17 },
            { file: "0021.mp3", duration: 39000, dfTrack: 21 },
            { file: "0022.mp3", duration: 239000, dfTrack: 22 },
