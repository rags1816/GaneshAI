...
<head>
    ...
    <!-- Real Noto Sans <Script> webfonts for every non-Latin language this
         ...
         Telugu, Gujarati, Punjabi/Gurmukhi, Malayalam, Bengali, Urdu/Farsi/
         Sindhi share Arabic, Thai, and Chinese each get their own family).
         Same fonts already used for the OLED's Indic rendering (see
         indic_fonts.h/renderTextToXbm in the backend) - without this, any
         non-Latin text on this page (a devotee's own name, a blessing
         reply) falls back to whatever the phone's own system font happens
         to cover, which is usually fine on modern phones but not
         guaranteed. Fails gracefully if this device's own AP-fallback
...
<body>
    <div class="puja-card">
        ...
        <!-- Shown instead of the form when a priest has turned offerings
             off from the dashboard (see FESTIVAL_ACTIVE_URL below) - e.g.
             once the festival itself has ended, so the QR code doesn't
             keep quietly accepting submissions indefinitely. -->
        <div id="closed-message" style="display:none; text-align:center; padding:30px 10px;">
            <p style="font-size:16px; color:var(--text-muted);">🙏 Bappa is not currently accepting offerings through this page. Please check back during puja hours.</p>
        </div>
        ...
        <div id="puja-form">
            ...
            <!-- 2. Standard Blessings Picker - a quick, zero-typing
                 alternative to the free-text wish below, for a devotee
                 who'd rather tap once than write/speak something specific.
                 Both can be filled in together (askClaudeForBlessing()
                 combines them); neither is required. -->
            <div class="form-group">
                <label for="standard-wish-select">Quick Blessing (Optional - or write your own below)</label>
                <select id="standard-wish-select" class="select-input">
                    <option value="">-- Pick a Quick Blessing --</option>
            ...
            <!-- 3. Reply language - picked first, so it's ready for the mic below.
                 Auto-detected from the wish's script if typed before this is
                 touched (see detectScriptLanguage() below) - a manual choice
                 here always wins over the auto-detect from then on. -->
            <div class="form-group">
                <label for="lang-select">Bappa's Reply Language</label>
                <select id="lang-select" class="select-input" onchange="langManuallySet = true; updateTextDirection();">
            ...
            <div class="form-group">
                ...
                <textarea id="wish-input" class="text-input" placeholder="Write your personal prayer here, or tap the mic to speak it..." oninput="updateWordCount(); autoDetectLanguage();"></textarea>
                <div class="count-row">
            ...
            <!-- Off by default - the priest queue is the real ritual: an
                 offering gets curated and offered, then Bappa's blessing
                 is heard from the temple itself when it's approved. This
                 checkbox is only for a devotee who also wants a private,
                 instant preview on their own phone the moment it's ready,
                 which happens well before any priest sees it. -->
            <div class="form-group" style="display:flex; align-items:center; gap:8px;">
                <input type="checkbox" id="phone-playback-opt-in" style="width:18px; height:18px; margin:0;">
    ...
    <script>
        ...
        // Admin on/off switch for this page (toggled from the priest
        // dashboard) - lets offerings be turned off, e.g. once the
        // festival has ended, without needing to take the page down or
        // reissue a new QR code. Missing/null means "on" (unset = the
        // page behaves exactly as it always has, no action needed to
        // keep accepting offerings).
        const FESTIVAL_ACTIVE_URL = 'https://ganapatiai-default-rtdb.europe-west1.firebasedatabase.app/festival_active.json';

        function checkFestivalActive() {
            fetch(`${FESTIVAL_ACTIVE_URL}?_t=${Date.now()}`)
        ...
        // Auto-detects the reply language from the wish's own script, so a
        // devotee who types/speaks in (say) Tamil doesn't also have to
        // remember to switch the dropdown away from its English default.
        // A manual dropdown choice always wins from then on - this only
        // ever acts before langManuallySet becomes true.
        let langManuallySet = false;
        const SCRIPT_RANGES = [
            // Devanagari covers both Hindi and Marathi - can't tell them
            // apart from script alone, so this defaults to the more
            // common of the two; a devotee who meant Marathi can still
            // pick it manually, which then sticks (see langManuallySet).
            { lang: 'hi', re: /[ऀ-ॿ]/ },
            { lang: 'ta', re: /[஀-௿]/ },
            { lang: 'te', re: /[ఀ-౿]/ },
            { lang: 'gu', re: /[઀-૿]/ },
            ...
            // actually on this page's dropdown.
            { lang: 'ur', re: /[؀-ۿݐ-ݿ]/ },
            { lang: 'th', re: /[฀-๿]/ },
            { lang: 'zh', re: /[一-鿿]/ }
        ];

        // Languages written right-to-left - only Urdu is reachable from
        // this page's dropdown today, but keeping this as a set (not a
        // single check) means Farsi/Sindhi get the same treatment for
        // free if they're ever added to the dropdown too.
        ...
        // Flips the wish textarea to right-to-left when the selected reply
        // language is written that way - without this, Urdu text a
        // devotee types renders left-aligned with the wrong reading order,
        // which is wrong for the script even though the characters
        // themselves are individually correct.
        ...
        function autoDetectLanguage() {
            if (!langManuallySet) {
                const wishInput = document.getElementById('wish-input');
                const detected = detectScriptLanguage(wishInput.value);
                if (detected) {
                    const langSelect = document.getElementById('lang-select');
                    if (langSelect.value !== detected) langSelect.value = detected;
                }
            }
            updateTextDirection();
        }
        ...
        function beginSpeechSession(wishInput, micBtn, myToken) {
            ...
            recognition.onresult = (event) => {
                try {
                    ...
                    wishInput.value = (speechAccumulatedText + sessionTranscript).trim();
                    updateWordCount(); // same 20-word limit/warning as typing
                    autoDetectLanguage();
                } catch (e) {
                    console.warn('onresult handler threw:', e);
                }
            };
            ...
            recognition.onend = () => {
                if (myToken !== speechSessionToken) return; // superseded by a later tap
        ...
        function submitPuja() {
            ...
            // backstop in case it somehow wasn't called (e.g. a paste
            // that skipped oninput), matching the same truncate-don't-
            // block behavior rather than surprising them with an alert.
            let words = wishInput.value.trim().split(/\s+/).filter(w => w.length > 0);
            if (words.length > 20) {
                words = words.slice(0, 20);
                wishInput.value = words.join(' ');
        ...
        // the AI-personalized version, if and when this finishes, and -
        // only if the devotee opted in - also plays the spoken reply right
        // here on their own phone as an instant private preview. Otherwise
        // the blessing is heard for the first time from the temple's own
        // speaker once a priest approves it from the queue - that's the
