...
exports.generateBlessing = onRequest(
    ...
    async (req, res) => {
      ...
      let blessingText;
      try {
        blessingText = await askClaudeForBlessing(name, offeringText, prayer);
      } catch (err) {
      ...
      let audioBuffer;
      try {
        audioBuffer = await synthesizeSpeech(blessingText);
      } catch (err) {
...
async function askClaudeForBlessing(name, offeringText, prayer) {
  const prompt = `You are Lord Ganesha, speaking warmly and directly to a devotee ` +
    ...
    `stage directions, no quotation marks, no markdown.`;
...
async function synthesizeSpeech(text) {
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      input: {text},
      // en-IN Wavenet male voice - adjust languageCode/name/ssmlGender to
      // taste once you've heard it; see Google's voice list for options.
